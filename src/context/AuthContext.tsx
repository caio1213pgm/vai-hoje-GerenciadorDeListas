    ...
    await signInWithPopup(auth, gitHubProvider)
      ...
      .catch((error) => {
        if (error.code === "auth/account-exists-with-different-credential") {
          const providerExist =
            error.customData._tokenResponse.verifiedProvider;
          return toast(
            `Já existe uma conta com este email no provider: ${providerExist}`,
          );
        }
      ...
      await signInWithEmailAndPassword(auth, data.email, data.password)
        ...
        .catch((error) => {
          ...
          if (error.code === "auth/user-not-found") {
            return toast("Usuário não encontrado. Verifique seu email.");
          }
